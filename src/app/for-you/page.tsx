...
function MatchBadge({ score }: { score: number }) {
  return (
    <span
      className="inline-flex items-center gap-0.5 text-[10px] font-black font-mono px-1.5 py-0.5 rounded-full"
      style={{
        color: "var(--accent)",
        background: "rgba(230,255,61,0.15)",
...
const ContinuitySection = memo(function ContinuitySection({
  items,
  onFeedback,
  onDetail,
  dismissedIds,
  ...
  const visible = items.filter(
    (i) => i.isContinuity && !dismissedIds.has(i.id),
  );
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-[10px] text-amber-400">
            Sequel e capitoli successivi dei tuoi titoli completati
          </p>
        </div>
      </div>
      <div className="flex gap-4 overflow-x-auto pb-3 scrollbar-hide">
        {visible.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className="flex-shrink-0 w-44 group relative cursor-pointer"
              onClick={() => onDetail?.(item)}
            >
              <div className="relative h-64 rounded-2xl overflow-hidden bg-zinc-900 mb-2">
                {item.coverImage ? (
                  <img
                    src={optimizeCover(item.coverImage, "foryou-card-small")}
                    alt={item.title}
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <Icon size={36} className="text-zinc-700" />
                  </div>
                )}
              ...
              <p className="text-xs font-bold text-white leading-tight line-clamp-2 mb-0.5">
                {item.title}
              </p>
            </div>
          );
  ...
  const signals = [
    item.isAwardWinner
      ? {
          ...
          label: "Award",
          icon: Trophy,
          tone: "text-amber-300 border-amber-400/25 bg-amber-500/12",
        }
      : null,
    item.isSeasonal
      ? {
          ...
          label: "Seasonal",
          icon: Calendar,
          tone: "text-sky-300 border-sky-400/25 bg-sky-500/12",
        }
      : null,
    item.isSerendipity
      ? {
          ...
          label: "Serendipity",
          icon: Compass,
          tone: "text-violet-300 border-violet-400/25 bg-violet-500/12",
        }
      : null,
    item.friendWatching
      ? {
          key: "friend",
          label: item.friendWatching,
          icon: Users,
          tone: "text-emerald-300 border-emerald-400/25 bg-emerald-500/12",
        }
      : null,
          ...
          <div className="absolute bottom-14 left-2 right-2 flex flex-wrap gap-1.5">
            {signals.slice(0, 1).map((signal) => {
              ...
              return (
                <span
                  key={signal.key}
                  className={`inline-flex min-w-0 items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-bold backdrop-blur-sm ${signal.tone}`}
                >
        ...
        <div className="absolute bottom-2 inset-x-0 flex items-center justify-center gap-2">
          ...
          <button
            ...
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              onFeedback(item, "already_seen");
            }}
            title="L'ho già visto"
          ...
          <button
            ...
            disabled={!onWishlist}
            title={
              wishlisted ? "Rimuovi dalla wishlist" : "Aggiungi alla wishlist"
            }
...
function SimilarSearchBar({
  ...
  const fetchSuggestions = async (q: string) => {
    ...
    try {
      ...
      const parse = (j: any) =>
        Array.isArray(j) ? j : j.results || j.data || [];
  ...
  return (
    <div ref={containerRef} className="relative mb-6">
      <div className="flex flex-col gap-2 lg:flex-row lg:items-center">
        ...
        <div className="relative min-w-0 flex-1">
          ...
          {query && (
            <button
              ...
              aria-label="Cancella ricerca simili"
            >
              <X size={13} />
            </button>
          )}
        ...
        {actions && (
          <div
            className="flex flex-shrink-0 items-center justify-end gap-2 lg:min-w-[240px]"
            data-no-swipe="true"
          >
            {actions}
          </div>
          ...
          {suggestions.map((s, i) => (
            ...
            >
              ...
              <div className="w-8 h-11 rounded-xl overflow-hidden bg-zinc-800 flex-shrink-0">
                {s.coverImage ? (
                  <img
                    src={optimizeCover(s.coverImage, "drawer-cover")}
                    alt=""
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs" style={{ color: "var(--accent)" }}>
                  {TYPE_LABEL_SEARCH[s.type] || s.type}
                  {s.year ? ` · ${s.year}` : ""}
                </p>
              </div>
            </button>
...
const SimilarSection = memo(function SimilarSection({
  ...
  const activeFilters = SIMILAR_TYPE_FILTERS.filter(
    (f) => f.key === "all" || presentTypes.has(f.key as MediaType),
  );
  ...
  return (
    <div
      className="mb-10 rounded-3xl p-5"
      style={{
        border: "1px solid rgba(230,255,61,0.2)",
        background: "rgba(230,255,61,0.03)",
      }}
    >
      {/* Header */}
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="text-sm font-bold text-white">
            Titoli simili a{" "}
            <span style={{ color: "var(--accent)" }}>"{sourceTitle}"</span>
          </h2>
          <p className="text-[10px] text-zinc-500">
            ...
            items.filter((i) => !dismissedIds.has(i.id)).length
              ? "titoli trovati"
              : `di ${items.filter((i) => !dismissedIds.has(i.id)).length} totali`}
          </p>
        </div>
        <button
          onClick={onClose}
        ...
        <div className="flex gap-2 overflow-x-auto pb-3 scrollbar-hide mb-4">
          {activeFilters.map(({ key, label }) => {
            const count =
              key === "all"
                ? items.filter((i) => !dismissedIds.has(i.id)).length
                : items.filter((i) => i.type === key && !dismissedIds.has(i.id))
                    .length;
              ...
              >
                {key !== "all" && (
                  <span
                    ...
                    style={{ background: TYPE_COLORS[key as MediaType] }}
                  />
                )}
                {label}
                <span
                  className={`text-[10px] ${isActive ? "" : "text-zinc-600"}`}
                  style={isActive ? { color: "rgba(11,11,15,0.7)" } : {}}
                >
                  {count}
                </span>
              </button>
      ...
      {filtered.length === 0 ? (
        <p className="text-sm text-zinc-500 text-center py-6">
          Nessun titolo trovato per questo filtro.
        </p>
      ) : (
        <div className="flex gap-4 overflow-x-auto pb-3 scrollbar-hide">
          {shown.map((item) => (
            ...
            <div className="flex-shrink-0 w-40 flex items-center justify-center">
              ...
              >
                ...
                <span className="text-[10px]">
                  +{filtered.length - visibleCount} altri
                </span>
              </button>
            </div>
          )}
    ...
    <div className="mb-10">
      <div className="flex items-center gap-3 mb-4">
        <div
          ...
          style={{ background: RAIL_COLORS[rail.kind] }}
        >
          <Icon
            size={16}
            className={
              rail.kind === "quick-picks" || rail.kind === "top-match"
                ? "text-black"
                : "text-white"
            }
          />
      ...
      <div className="flex gap-4 overflow-x-auto pb-3 scrollbar-hide">
        {shown.map((item) => (
          <RecommendationCard
            ...
            dismissed={dismissedIds.has(item.id)}
            showDetails={
              rail.kind === "top-match" || rail.kind === "because-title"
            }
            added={addedIds.has(item.id)}
            wishlisted={wishlistIds.has(item.id)}
            adding={addingIds.has(item.id)}
        ...
        {hasMore && (
          <div className="flex-shrink-0 w-40 flex items-center justify-center">
            ...
            >
              ...
              <span className="text-[10px]">
                +{visible.length - visibleCount} altri
              </span>
            </button>
          </div>
        )}
...
const SpotlightRecommendation = memo(function SpotlightRecommendation({
  item,
  onFeedback,
  onSimilar,
  onDetail,
      ...
      <div className="relative z-10 flex min-h-[190px] md:min-h-[230px] items-end p-4 md:p-6">
        <div className="max-w-2xl">
          <div className="flex items-center gap-2 mb-3">
            <span
              className="inline-flex items-center gap-1.5 text-[10px] font-bold text-white px-2 py-1 rounded-full"
              style={{ background: colorClass }}
            >
          ...
          <h1 className="text-2xl md:text-4xl font-black text-white leading-tight mb-2 line-clamp-2">
            {item.title}
          </h1>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              onClick={() => onFeedback(item, "not_interested")}
...
function QuickReasonSheet({
  ...
  return (
    ...
    >
      ...
      <div
        className="relative w-full max-w-sm bg-zinc-950 border border-zinc-800 rounded-3xl p-5"
        onClick={(e) => e.stopPropagation()}
      >
        <p className="text-xs font-semibold text-zinc-500 uppercase tracking-widest mb-1">
          Perché non ti interessa?
        </p>
        <p className="text-sm font-semibold text-white mb-5 truncate">
          {item.title}
        </p>
    ...
    <div className="flex items-start gap-3 bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4 mb-6">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-amber-300 mb-1">
          Consigli in miglioramento
        </p>
        <p className="text-xs text-amber-200/70 mb-3">
          I tuoi consigli migliorano man mano che aggiungi titoli. Hai ancora{" "}
          {needed - totalEntries} titoli per sbloccare i consigli
          personalizzati.
        </p>
        ...
        <Link
          href="/discover"
          className="inline-flex items-center gap-1.5 mt-3 text-xs font-semibold text-amber-400 hover:text-amber-300"
        >
          <Plus size={13} />
        ...
        {items.map((a) => (
          ...
          >
            <div className="relative h-40 rounded-2xl overflow-hidden bg-zinc-800 mb-2">
              {a.mediaCover ? (
                <img
                  src={optimizeCover(a.mediaCover, "foryou-friend")}
                  alt={a.mediaTitle}
            ...
            <p className="text-[10px] font-semibold text-zinc-300 line-clamp-2 mb-0.5">
              {a.mediaTitle}
            </p>
            <p
...
(forYouCache as any).locale = null;
...
function SwipeModeWrapper({ onClose }: { onClose: () => void }) {
  ...
  const sendSwipeFeedback = async (
    ...
    if (
      (action === "already_seen" || action === "added") &&
  ...
  const requestMore = async (): Promise<SwipeItem[]> => {
    const res = await fetch(
      `/api/recommendations?type=all&lang=${locale}&refresh=1`,
      { credentials: "include" },
    ).catch(() => null);
    if (!res?.ok) return [];
    const json = await res.json();
  ...
  return (
    <SwipeMode
      ...
      onSeen={(item, rating) =>
        sendSwipeFeedback(item, "already_seen", rating ?? undefined)
      }
...
export default function ForYouPage() {
  ...
  const hasCachedData =
    ...
    (forYouCache as any).locale === locale;
  const [loading, setLoading] = useState(!hasCachedData);
  const [refreshing, setRefreshing] = useState(false);
  const [recommendations, setRecommendations] = useState<
    Record<string, Recommendation[]>
  >(hasCachedData ? (forYouCache.recommendations ?? {}) : {});
  const [rails, setRails] = useState<RecommendationRail[]>(
    hasCachedData ? (forYouCache.rails ?? []) : [],
  );
  const [tasteProfile, setTasteProfile] = useState<TasteProfile | null>(
    hasCachedData ? forYouCache.tasteProfile : null,
  );
  ...
  const [friendsLoading, setFriendsLoading] = useState(
    !hasCachedData || forYouCache.friendsActivity.length === 0,
  );
  const [addingIds, setAddingIds] = useState<Set<string>>(new Set());
  const [reasonPending, setReasonPending] = useState<Recommendation | null>(
    null,
  );
  ...
  const [similarSection, setSimilarSection] = useState<{
    sourceTitle: string;
    sourceType: MediaType;
    items: Recommendation[];
  } | null>(null);
  ...
  const fetchRecommendations = useCallback(
    async (force = false) => {
      ...
      const res = await fetch(
        `/api/recommendations?type=all&lang=${locale}${force ? "&refresh=1" : ""}`,
      );
      if (!res.ok) return;
      const json = await res.json();
  ...
  const fetchFriends = useCallback(
    async (userId: string) => {
      ...
      try {
        ...
        const highSimIds = new Set(
          (simData || []).map((s: any) => s.other_user_id),
        );
        ...
        for (const e of entries) {
          ...
          activity.push({
            ...
            mediaId: e.external_id,
            mediaTitle: e.title,
            mediaCover: e.cover_image,
            mediaType: e.type,
            updatedAt: e.updated_at,
            isHighSim: highSimIds.has(e.user_id),
            simScore: simMap[e.user_id] || 0,
          });
        }
    ...
    const init = async () => {
      ...
      if (isActive) {
        ...
        const existing = supabase
          .getChannels()
          .find((c) => c.topic === `realtime:${chName}`);
        if (!existing) {
          profileChannel = supabase
            ...
            .on(
              ...
              (payload: any) => {
                setTotalEntries(payload.new?.entry_count ?? 0);
              },
            )
            .subscribe();
        }
      ...
      if (
        ...
        Promise.all([
          ...
          const newAddedIds: Set<string> = new Set(
            (entries || [])
              .map((e: any) => e.external_id as string)
              .filter(Boolean),
          );
          const newTitles: Set<string> = new Set(
            (entries || [])
              .map((e: any) => (e.title as string)?.toLowerCase())
              .filter(Boolean),
          );
          const newWishIds: Set<string> = new Set(
            (wish || [])
              .map((w: any) => w.external_id as string)
              .filter(Boolean),
          );
          setAddedIds(newAddedIds);
          setWishlistIds(newWishIds);
          setTotalEntries(entries?.length || 0);
          addedTitlesRef.current = newTitles;
          forYouCache.addedIds = newAddedIds;
          forYouCache.wishlistIds = newWishIds;
          forYouCache.addedTitles = newTitles;
          forYouCache.totalEntries = entries?.length || 0;
        });
        fetchFriends(userId);
        return;
      }
      ...
      const newWishIds: Set<string> = new Set(
        (wish || []).map((w: any) => w.external_id as string).filter(Boolean),
      );
      setAddedIds(newAddedIds);
      setWishlistIds(newWishIds);
      setTotalEntries(entries?.length || 0);
      ...
      const poolRes = await fetch(
        `/api/recommendations?source=pool&lang=${locale}`,
        { cache: "no-store" },
      );
      if (poolRes.ok) {
        const poolJson = await poolRes.json();
        if (
          (poolJson.source === "pool" ||
            poolJson.source === "pool_master_sample") &&
          poolJson.recommendations
        ) {
          ...
          setIsCached(true);
          setRails(nextRails);
          forYouCache.recommendations = recs;
          forYouCache.rails = nextRails;
          forYouCache.tasteProfile = poolJson.tasteProfile || null;
          (forYouCache as any).locale = locale;
      ...
      const recsRes = await fetch(
        `/api/recommendations?type=all&lang=${locale}`,
      );
      if (recsRes.ok) {
        ...
        for (const [type, items] of Object.entries(incoming)) {
          if (Array.isArray(items) && (items as any[]).length > 0)
            merged[type] = items as Recommendation[];
        }
        ...
        forYouCache.rails = nextRails;
        forYouCache.tasteProfile = json.tasteProfile || null;
        (forYouCache as any).locale = locale;
        forYouCache.ts = Date.now();
      }
  ...
  const handleRefresh = async () => {
    ...
    const [lightJson] = await Promise.all([
      fetch(`/api/recommendations?type=all&lang=${locale}&refresh=1`, {
        cache: "no-store",
      })
    ...
    if (json && json.recommendations) {
      ...
      if (json.tasteProfile) {
        setTasteProfile(json.tasteProfile);
        forYouCache.tasteProfile = json.tasteProfile;
      }
      setIsCached(false);
    }
    setRefreshing(false);
    ...
    async (item: Recommendation) => {
      ...
      const res = await fetch("/api/collection", {
        ...
        body: JSON.stringify({
          ...
          status:
            item.type === "movie" || isBoardgame ? "completed" : "watching",
      ...
      if (wishlistIds.has(item.id)) {
        ...
        const res = await fetch("/api/wishlist", {
          ...
          body: JSON.stringify({
            external_id: item.id,
            title: item.title,
            title_original: (item as any).title_original || item.title,
            title_en: (item as any).title_en || item.title,
            title_it: (item as any).title_it || null,
  ...
  const handleDetail = useCallback((item: Recommendation) => {
    const details: MediaDetails = {
      ...
      source: item.id.startsWith("anilist-anime")
        ? "anilist"
        : item.id.startsWith("anilist-manga")
          ? "anilist"
          : item.id.startsWith("tmdb-")
            ? "tmdb"
            : item.id.startsWith("igdb-")
              ? "igdb"
              : item.id.startsWith("ol-")
                ? "ol"
                : item.id.startsWith("bgg-")
                  ? "bgg"
                  : item.type === "boardgame"
                    ? "bgg"
                    : /^\d+$/.test(item.id) && item.type === "game"
                      ? "igdb"
                      : /^\d+$/.test(item.id) &&
                          (item.type === "movie" ||
                            item.type === "tv" ||
  ...
  const searchSimilar = useCallback(
    async (
      ...
      if (keywords?.length)
        params.set("keywords", keywords.slice(0, 15).join(","));
      if (excludeId) params.set("excludeId", excludeId);
      if (type) params.set("type", type);
      const res = await fetch(`/api/recommendations/similar?${params}`);
      if (res.ok) {
        const json = await res.json();
        const items: Recommendation[] = (json.items || []).filter(
          (r: Recommendation) => r.id !== excludeId,
        );
        setSimilarSection({
          sourceTitle: title,
          sourceType: (type as MediaType) || "movie",
          items,
        });
        scrollToTop("smooth");
      } else {
  ...
  const handleFeedback = useCallback(
    (item: Recommendation, action: FeedbackAction, reason?: FeedbackReason) => {
  ...
  const displayRecs = Object.fromEntries(
    Object.entries(recommendations).map(([type, items]) => [
      type,
      (items as Recommendation[]).map((r) => ({ ...r })),
    ]),
  ...
  const displayRailsWithFriends = rails.map((rail) => ({
    ...
    items: rail.items.map((rec) => {
      ...
      return {
        ...rec,
        friendWatching: friendWatchingMap.get(rec.id),
        matchScore: Math.min(100, rec.matchScore + 12),
      };
    }),
  }));
  ...
  const visibleRails = displayRailsWithFriends
    .map((rail) => ({
      ...rail,
      items: rail.items.filter((i) => !dismissedIds.has(i.id)),
    }))
    .filter((rail) => rail.items.length > 0);
  const spotlightItem =
    visibleRails.find((rail) => rail.kind === "top-match")?.items[0] ||
    visibleRails[0]?.items[0];
    ...
    <div className="gk-for-you-page min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)]">
      ...
      <div className="pt-2 md:pt-8 pb-24 max-w-screen-2xl mx-auto px-3 sm:px-4 md:px-6">
        {!hasEnoughData ? (
          ...
          <>
            {totalEntries < 15 && (
              <LowConfidenceBanner totalEntries={totalEntries} />
            )}
            {tasteProfile && (
              <DNAWidget
            ...
            <SimilarSearchBar
              onSearch={(title, genres, keywords, type) =>
                searchSimilar(
                  title,
                  genres,
                  undefined,
              ...
              actions={
                <>
                  ...
                  <div className="relative">
                    ...
                    {showNewRecsBadge && (
                      <span
                        className="absolute -right-0.5 -top-0.5 h-2.5 w-2.5 animate-pulse rounded-full border border-black"
                        style={{ background: "var(--accent)" }}
                      />
                    )}
                  </div>
            ...
            {visibleRails.length === 0 &&
              SECTIONS.every(({ key }) => {
                const items = (displayRecs[key] || []).filter(
