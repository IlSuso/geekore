...
async function fetchAnimeQuick(token: string, locale: 'it' | 'en'): Promise<any[]> {
  if (!token) return []
  const results: any[] = []
  ...
  await Promise.all(pages.map(async (page) => {
    try {
      const params = new URLSearchParams({
        ...
        'vote_count.gte': '100',
        'vote_average.gte': '6',
        language: locale === 'it' ? 'it-IT' : 'en-US',
        page: String(page),
      })
      ...
      for (const m of (json.results || [])) {
        ...
        results.push({
          ...
          why: locale === 'it' ? 'Popolare tra gli appassionati di anime' : 'Popular among anime fans',
...
async function fetchMovieQuick(token: string, locale: 'it' | 'en'): Promise<any[]> {
  if (!token) return []
  const results: any[] = []
  const seen = new Set<string>()
  ...
  await Promise.all([
    ...
    try {
      const params = new URLSearchParams({ sort_by: sort, 'vote_count.gte': '200', 'vote_average.gte': '6.5', language: locale === 'it' ? 'it-IT' : 'en-US', page: String(page) })
...
async function fetchTvQuick(token: string, locale: 'it' | 'en'): Promise<any[]> {
  if (!token) return []
  const results: any[] = []
  const seen = new Set<string>()
  ...
  await Promise.all([
    ...
    try {
      const params = new URLSearchParams({ sort_by: sort, without_genres: '16', without_keywords: '210024', 'vote_count.gte': '100', 'vote_average.gte': '6.5', language: locale === 'it' ? 'it-IT' : 'en-US', page: String(page) })
      ...
      for (const m of (json.results || [])) {
        ...
        seen.add(id)
        const year = m.first_air_date ? parseInt(m.first_air_date.slice(0, 4)) : undefined
        results.push({ id, title: m.name || 'Senza titolo', type: 'tv', coverImage: `https://image.tmdb.org/t/p/w780${m.poster_path}`, year, genres: [], score: m.vote_average ? Math.min(m.vote_average / 2, 5) : undefined, description: m.overview || undefined, why: locale === 'it' ? 'Serie TV molto seguita' : 'Popular TV show', matchScore: Math.round((m.popularity || 0) / 10) })
      }
    } catch {}
  }))
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const fetchMap: Record<string, () => Promise<any[]>> = {
      anime: () => fetchAnimeQuick(tmdbToken, locale),
    ...
    const payload = await localizeRecommendationPayload({ recommendations, source: 'onboarding_quick', cached: false }, locale)
