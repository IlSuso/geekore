...
export async function GET(request: NextRequest) {
  ...
  for (const mediaType of types) {
    try {
      ...
      const responses = await Promise.all(
        searchTerms.map((t) =>
          fetch(ANILIST_API, {
            ...
            body: JSON.stringify({
              query: QUERY,
              variables: { search: t, type: mediaType },
            }),
      ...
      const media: any[] = responses.flatMap(
        (json: any) => json.data?.Page?.media || [],
      );
      ...
      allResults.push(
        ...media
          ...
          .map((m: any) => {
            ...
            const studios: string[] = (m.studios?.nodes || [])
              .map((s: any) => s.name)
              .filter(Boolean);
            ...
            const descriptionEn = m.description
  ...
  if (lang === "it") {
    ...
    if (toTranslate.length > 0) {
      const items = toTranslate.map((r: any) => ({
        id: r.id,
        text: r.description,
      }));
