          ...
          .map(async (m: any) => {
            ...
            if (mediaType === "tv") {
              try {
                const [detailRes, kwRes] = await Promise.all([
                  fetch(`${TMDB_BASE}/tv/${m.id}?language=${tmdbLang}`, {
                    headers: tmdbHeaders(),
                    signal: AbortSignal.timeout(3000),
                  }),
                  fetch(`${TMDB_BASE}/tv/${m.id}/keywords`, {
                    headers: tmdbHeaders(),
                    signal: AbortSignal.timeout(3000),
                  }),
                ]);
            ...
            return {
              ...
              year: m.release_date
                ...
                : m.first_air_date
                  ? parseInt(m.first_air_date.substring(0, 4))
                  : undefined,
              description,
              ...
              score: m.vote_average
                ? Math.min(Math.round(m.vote_average * 10) / 20, 5)
                : undefined,
              source: "tmdb",
            };
          }),
