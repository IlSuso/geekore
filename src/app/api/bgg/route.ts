...
async function fetchBGGBatch(ids: string[]): Promise<BGGItem[]> {
  ...
  while ((m = itemRe.exec(xml)) !== null) {
    ...
    // Giocatori e tempo
    const minPlayers =
      parseInt(extractAttr(chunk, "minplayers", "value")) || undefined;
    const maxPlayers =
      parseInt(extractAttr(chunk, "maxplayers", "value")) || undefined;
    const playingTime =
      parseInt(extractAttr(chunk, "playingtime", "value")) || undefined;

    // Rating BGG normalizzato su /5 (BGG usa scala 1-10)
    const ratingM = chunk.match(/<average[^>]*value="([\d.]+)"/);
    const score = ratingM
      ? Math.round((parseFloat(ratingM[1]) / 2) * 10) / 10
      : undefined;

    // Complessità/weight (da 1 a 5)
    const weightM = chunk.match(/<averageweight[^>]*value="([\d.]+)"/);
    const complexity = weightM
      ? Math.round(parseFloat(weightM[1]) * 10) / 10
      : undefined;
  ...
  try {
    ...
    if (locale === "it") {
      ...
      if (toTranslate.length > 0) {
        const descItems = toTranslate.map((r) => ({
          id: r.id,
          text: r.description!,
        }));
