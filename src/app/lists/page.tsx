...
import {
  ...
  ChevronRight, Loader2, Search, Sparkles, Trophy, Film, Gamepad2,
} from 'lucide-react'
...
const LIST_TEMPLATES = [
  { icon: <Trophy size={16} />, title: 'Top personali', text: 'Classifiche dei tuoi preferiti per ogni medium.' },
  { icon: <Film size={16} />, title: 'Serate film', text: 'Raccolte pronte da condividere con amici.' },
  { icon: <Gamepad2 size={16} />, title: 'Backlog', text: 'Giochi, anime e serie da recuperare senza perderli.' },
]
      ...
      <div className="w-full max-w-md overflow-hidden rounded-[30px] border border-[rgba(230,255,61,0.18)] bg-[var(--bg-primary)] shadow-[0_24px_80px_rgba(0,0,0,0.55)]">
        <div className="border-b border-[var(--border)] bg-[rgba(230,255,61,0.04)] p-5">
          <div className="mb-2 flex items-center justify-between gap-4">
            ...
            <button type="button" data-no-swipe="true" onClick={onClose} className="flex h-9 w-9 items-center justify-center rounded-2xl border border-[var(--border)] bg-black/20 text-[var(--text-secondary)] hover:text-white" aria-label="Chiudi modal lista">
              <X size={17} />
            </button>
          </div>
        ...
        <div className="flex gap-3 border-t border-[var(--border)] p-5">
          ...
          <button
            ...
            style={canSave ? { background: 'var(--accent)', color: '#0B0B0F' } : undefined}
          >
...
function ListCard({
  ...
  return (
    <div className="group overflow-hidden rounded-[24px] border border-[var(--border-subtle)] bg-[var(--bg-card)] transition-colors hover:border-[rgba(230,255,61,0.26)] hover:bg-[var(--bg-card-hover)]">
      <Link href={`/lists/${list.id}`} data-no-swipe="true" className="block p-4">
        <div className="flex items-center gap-3">
          <div className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-2xl border border-[rgba(230,255,61,0.18)] bg-[linear-gradient(135deg,rgba(230,255,61,0.16),rgba(255,255,255,0.03))] text-[var(--accent)]">
            <List size={24} />
          </div>
          <div className="min-w-0 flex-1">
            <div className="mb-1 flex items-center gap-2">
              ...
              <h3 className="line-clamp-1 text-[16px] font-black text-[var(--text-primary)]">{list.title}</h3>
            </div>
            {list.description ? (
      ...
      <div className="flex gap-2 border-t border-[var(--border-subtle)] px-4 py-3" data-no-swipe="true">
        <button type="button" data-no-swipe="true" onClick={() => onEdit(list)} className="flex items-center gap-1.5 rounded-xl px-2 py-1 text-xs font-bold text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-secondary)] hover:text-[var(--text-primary)]">
          <Edit3 size={12} /> Modifica
        </button>
        <button type="button" data-no-swipe="true" onClick={() => onDelete(list.id)} className="ml-auto flex items-center gap-1.5 rounded-xl px-2 py-1 text-xs font-bold text-[var(--text-muted)] transition-colors hover:bg-red-500/10 hover:text-red-400">
    ...
    >
      <div className="mb-5 overflow-hidden rounded-[28px] border border-[rgba(230,255,61,0.16)] bg-[linear-gradient(135deg,rgba(230,255,61,0.075),rgba(18,18,26,0.96))] p-5 ring-1 ring-white/5">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <div className="mb-2 gk-section-eyebrow"><List size={13} /> Raccolte</div>
            <h1 className="font-display text-[34px] font-black leading-none tracking-[-0.045em] text-[var(--text-primary)] md:text-[42px]">Liste</h1>
            <p className="mt-2 max-w-2xl text-[14px] leading-6 text-[var(--text-secondary)]">Costruisci collezioni, classifiche e percorsi di visione da tenere sul profilo o condividere con la community.</p>
          </div>
          <button type="button" data-no-swipe="true" onClick={() => { setEditingList(undefined); setShowModal(true) }} className="inline-flex h-10 flex-shrink-0 items-center justify-center gap-2 rounded-2xl px-4 text-sm font-black transition-transform hover:scale-[1.02]" style={{ background: 'var(--accent)', color: '#0B0B0F' }}>
      ...
      <div className="relative mb-5">
        ...
        <input data-no-swipe="true" value={query} onChange={event => setQuery(event.target.value)} placeholder="Cerca liste, temi, descrizioni..." className="w-full rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] py-2.5 pl-10 pr-4 text-[14px] text-[var(--text-primary)] outline-none placeholder:text-[var(--text-muted)] transition-colors focus:border-[rgba(230,255,61,0.45)]" />
      </div>

      {loading ? (
        <div className="space-y-2">{Array.from({ length: 5 }).map((_, index) => <div key={index} className="h-[98px] rounded-2xl bg-[var(--bg-card)] skeleton" />)}</div>
      ) : filtered.length === 0 ? (
        <div className="rounded-[28px] border border-[var(--border)] bg-[var(--bg-card)] px-6 py-12 text-center">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-3xl border border-[var(--border)] bg-[var(--bg-secondary)]"><List size={28} className="text-[var(--text-muted)]" /></div>
          <p className="gk-headline mb-1 text-[var(--text-primary)]">{lists.length === 0 ? 'Nessuna lista ancora' : 'Nessuna lista trovata'}</p>
          <p className="gk-body mx-auto mb-5 max-w-sm">{lists.length === 0 ? 'Crea la tua prima lista oppure parti da un template mentale: top, backlog, serata film o preferiti di sempre.' : 'Prova a cambiare ricerca.'}</p>
          {lists.length === 0 && (
            <div className="mx-auto mb-6 grid max-w-3xl gap-3 md:grid-cols-3">
              {LIST_TEMPLATES.map(template => (
                <div key={template.title} className="rounded-2xl border border-[var(--border-subtle)] bg-black/18 p-4 text-left ring-1 ring-white/5">
                  <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-xl bg-[rgba(230,255,61,0.10)] text-[var(--accent)]">{template.icon}</div>
                  <p className="text-sm font-black text-[var(--text-primary)]">{template.title}</p>
                  <p className="mt-1 text-xs leading-5 text-[var(--text-muted)]">{template.text}</p>
                </div>
              ))}
            </div>
          )}
          <button type="button" data-no-swipe="true" onClick={() => { lists.length === 0 ? setShowModal(true) : setQuery('') }} className="inline-flex h-10 items-center justify-center rounded-2xl bg-[var(--accent)] px-4 text-sm font-black text-[#0B0B0F] transition-transform hover:scale-[1.02]">
            {lists.length === 0 ? 'Crea la prima lista' : 'Cancella ricerca'}
          </button>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map(list => <ListCard key={list.id} list={list} onEdit={l => { setEditingList(l); setShowModal(true) }} onDelete={handleDelete} />)}
        </div>
      )}

      {showModal && <ListModal list={editingList} onClose={() => { setShowModal(false); setEditingList(undefined) }} onSaved={handleSaved} />}
    </PageScaffold>
  )
}
