...
const PostCard = memo(function PostCard({
  ...
  return (
    <div className={`rounded-2xl transition-all duration-300 animate-fade-in ${
      ...
      <div className="flex items-center gap-3 px-5 pt-4 pb-2.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Category badge — spostato inline con il nome */}
            {post.category && (
      ...
      {/* Testo del post — headline + body separati */}
      <div className="px-5 pb-3">
        {(() => {
          ...
          return (
            <>
              {headline && (
                <p className="font-bold text-[var(--text-primary)] text-[16px] leading-snug tracking-tight mb-1">
                  {headline}
                </p>
              )}
              {rest && (
                <p className="text-[var(--text-secondary)] text-[14px] leading-relaxed whitespace-pre-wrap">
                  {rest}
                </p>
              )}
            </>
      ...
      {/* Categoria — rimossa da qui, spostata nell'header */}
      ...
      <div className="px-5 py-2.5 flex items-center gap-6 border-t border-zinc-800/50">
        ...
        <button
          onClick={async () => {
            ...
            if (navigator.share) {
              await navigator.share({ title: 'Geekore', text: post.content.slice(0, 80), url })
            } else {
...
export default function FeedPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)]">
            ...
            <div className="flex flex-col gap-3 pt-3">
              {displayedPosts.length === 0 ? (
                categoryFilter ? (
                  <EmptyState
                    icon={Sparkles}
                    title={`Nessun post per "${parseCategoryString(categoryFilter)?.subcategory || categoryFilter}"`}
                    description="Sii il primo a pubblicare in questa categoria!"
                    action={{ label: 'Rimuovi filtro', onClick: () => setCategoryFilter('') }}
                    accent="zinc"
                  />
                ) : feedFilter === 'following' ? (
                  <EmptyState
                    icon={Sparkles}
                    title={f.noFollowingTitle}
                    description={f.noFollowingHint}
                    action={{ label: 'Scopri utenti →', href: '/discover' }}
                  />
                ) : (
                  <EmptyState
                    icon={Sparkles}
                    title={f.emptyTitle}
                    description={f.emptyHint}
                    action={{ label: 'Scopri utenti →', href: '/discover' }}
                  />
                )
              ) : (
      ...
      {/* FAB — Floating Action Button nuovo post, solo mobile */}
      {currentUser && (
        <button
