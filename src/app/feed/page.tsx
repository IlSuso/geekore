...
import {
  ...
  Loader2, Pin, ArrowUp, Trash2, Tag, ChevronDown, MoreHorizontal,
  Check, PartyPopper, Bell, ChevronRight, Flame
} from 'lucide-react'
...
import {
  MACRO_CATEGORIES, CategoryIcon, CategoryBadge, CategorySelector,
  CategoryFilter, parseCategoryString
} from '@/components/feed/CategoryComponents'
...
async function trackAffinity(supabase: ReturnType<typeof import('@/lib/supabase/client').createClient>, userId: string, category: string | null | undefined) {
  if (!category) return
...
const PostCard = memo(function PostCard({
  ...
  post: Post; currentUser: User | null; currentProfile: { username: string; display_name?: string; avatar_url?: string } | null; isLiking: boolean
                      ...
                      >
                          ...
                          <div className="flex gap-3 px-5 pt-5 pb-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <textarea data-testid="post-composer" autoFocus value={newPostContent}
                                onChange={e => { setNewPostContent(e.target.value.slice(0, 2200)); const el = e.target; el.style.height = 'auto'; el.style.height = el.scrollHeight + 'px' }}
                                placeholder={f.placeholder} maxLength={2200} rows={3}
                        ...
                        <div className="flex-shrink-0 border-t border-zinc-800/60 px-4 py-3 flex items-center gap-3" style={{ background: 'var(--bg-primary)' }}>
                          ...
                          <div className="ml-auto flex items-center gap-2.5">
                            {newPostContent.length >= 1800 && <span className={`text-[12px] font-semibold ${newPostContent.length >= 2150 ? 'text-red-400' : 'text-zinc-500'}`}>{2200 - newPostContent.length}</span>}
                            {newPostContent.length > 0 && <svg width="26" height="26" viewBox="0 0 26 26"><circle cx="13" cy="13" r="10" fill="none" stroke="#27272a" strokeWidth="2.5" /><circle cx="13" cy="13" r="10" fill="none" stroke={newPostContent.length >= 2150 ? '#f87171' : newPostContent.length >= 1800 ? '#fb923c' : '#7c3aed'} strokeWidth="2.5" strokeDasharray={`${(newPostContent.length / 2200) * 62.83} 62.83`} strokeLinecap="round" transform="rotate(-90 13 13)" /></svg>}
                          </div>
                        </div>
                      </div>
                    )}
                      ...
                      <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', zIndex: 9999, background: 'var(--bg-primary)', display: 'flex', flexDirection: 'column' }}>
                        ...
                        <div style={{ flexShrink: 0, borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', paddingBottom: 'max(10px, env(safe-area-inset-bottom))', background: 'var(--bg-primary)' }}>
                          ...
                          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 10 }}>
                            {newPostContent.length >= 1800 && <span style={{ fontSize: 12, fontWeight: 600, color: newPostContent.length >= 2150 ? '#f87171' : '#71717a' }}>{2200 - newPostContent.length}</span>}
                            {newPostContent.length > 0 && <svg width="26" height="26" viewBox="0 0 26 26"><circle cx="13" cy="13" r="10" fill="none" stroke="#27272a" strokeWidth="2.5" /><circle cx="13" cy="13" r="10" fill="none" stroke={newPostContent.length >= 2150 ? '#f87171' : newPostContent.length >= 1800 ? '#fb923c' : '#7c3aed'} strokeWidth="2.5" strokeDasharray={`${(newPostContent.length / 2200) * 62.83} 62.83`} strokeLinecap="round" transform="rotate(-90 13 13)" /></svg>}
                          </div>
                        </div>
                      </div>,
