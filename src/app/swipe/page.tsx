...
function rowToSwipeItem(row: any): SwipeItem {
  return {
    ...
    source: row.source,
    ...(row.title_original
      ? ({ title_original: row.title_original } as any)
      : {}),
    ...(row.title_en ? ({ title_en: row.title_en } as any) : {}),
    ...(row.title_it ? ({ title_it: row.title_it } as any) : {}),
    ...(row.description_en
      ? ({ description_en: row.description_en } as any)
      : {}),
    ...(row.description_it
      ? ({ description_it: row.description_it } as any)
      : {}),
    ...(row.localized ? ({ localized: row.localized } as any) : {}),
  };
}
...
function toQueueRow(r: any, userId: string) {
  return {
    ...
    description_it: r.description_it ?? null,
  ...
  useEffect(() => {
    async function init() {
      ...
      if (existingRows.length >= 10) {
        setInitialItems(
          await localizeSwipeItems(existingRows.map(rowToSwipeItem), locale),
        );
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        if (res.ok) {
          ...
          setInitialItems(await localizeSwipeItems(combined, locale));
        }
      } catch {}
  ...
  const handleSwipeRequestMore = useCallback(
    async (filter: string = "all"): Promise<SwipeItem[]> => {
      ...
      try {
        ...
        return await localizeSwipeItems(combined, locale);
      } catch {
        return await localizeSwipeItems(
          existingRows.map(rowToSwipeItem),
          locale,
        ...
        <div className="flex flex-col items-center gap-5 text-center">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-sm mt-1">
              Sto cercando i titoli migliori per te…
            </p>
          </div>
        </div>
      </div>
