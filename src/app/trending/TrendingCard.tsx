...
function proxyCoverUrl(src: string | null | undefined): string | null {
  if (!src || !src.startsWith('https://') || src.includes('/api/image-proxy?')) return null
  return `/api/image-proxy?url=${encodeURIComponent(src)}`
}

function retryWithProxy(event: SyntheticEvent<HTMLImageElement>) {
  const img = event.currentTarget
  const proxied = proxyCoverUrl(img.currentSrc || img.src)
  if (proxied && img.dataset.proxyTried !== '1') {
...
function toDrawerMedia(item: TrendingItem): MediaDetails {
  const type = normalizeType(item.type)
  const achievementData = (item as any).achievement_data
  const achievement = achievementData && typeof achievementData === 'object' ? achievementData : null
  const bgg = achievement?.bgg && typeof achievement.bgg === 'object' ? achievement.bgg : null

  return {
    id: item.external_id || `${type}:${item.title}`,
    external_id: item.external_id || undefined,
    title: item.title,
    title_en: (item as any).title_en,
    title_it: (item as any).title_it,
    title_original: (item as any).title_original,
    type,
    coverImage: item.cover_image || (item as any).coverImage || undefined,
    ...
    genres: Array.isArray((item as any).genres) ? (item as any).genres : undefined,
    year: (item as any).year ?? (item as any).release_year,
    episodes: (item as any).episodes,
    seasons: (item as any).season_episodes,
    score: (item as any).score ?? item.avg_rating ?? bgg?.score,
    min_players: (item as any).min_players ?? bgg?.min_players,
    max_players: (item as any).max_players ?? bgg?.max_players,
    playing_time: (item as any).playing_time ?? bgg?.playing_time,
    complexity: (item as any).complexity ?? bgg?.complexity,
    mechanics: (item as any).mechanics,
    designers: (item as any).designers,
    authors: (item as any).authors,
    studios: (item as any).studios,
    directors: (item as any).directors,
    developers: (item as any).developer ? [(item as any).developer] : (item as any).developers,
  } as MediaDetails
}

const LOCALIZE_OPTIONS = {
  titleKeys: ['title'],
  coverKeys: ['cover_image', 'coverImage'],
...
export function TrendingHeroCard({ item, rank }: { item: TrendingItem; rank: number }) {
  ...
  const localizedItem = useLocalizedMediaRow(item, LOCALIZE_OPTIONS) || item
  const type = normalizeType(localizedItem.type)
  const Icon = TYPE_ICON[type] || Film
  const rating = scoreLabel(localizedItem)
  const rankLabel = rank === 0 ? copy.trendLeader : rank === 1 ? copy.rising : copy.hotNow
  const drawerPayload = useMemo(() => toDrawerMedia({ ...localizedItem, type }), [localizedItem, type])

  return (
    <>
      <button
        type="button"
        onClick={() => setDrawerMedia(drawerPayload)}
        data-no-swipe="true"
        className={`group relative grid min-h-[230px] w-full overflow-hidden rounded-[28px] border bg-[var(--bg-card)] p-4 text-left transition-all hover:scale-[1.01] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]/35 md:grid-cols-[150px_1fr] md:gap-4 ${rank === 0 ? 'md:col-span-2 border-[rgba(230,255,61,0.28)] md:grid-cols-[190px_1fr]' : 'border-[var(--border-subtle)]'}`}
        aria-label={localizedItem.title}
      >
        {localizedItem.cover_image ? (
          <img src={localizedItem.cover_image} alt="" className="absolute inset-0 h-full w-full object-cover opacity-24 blur-xl scale-110" loading={rank === 0 ? 'eager' : 'lazy'} onError={retryWithProxy} />
        ) : null}
        <div className="absolute inset-0 bg-gradient-to-r from-black/86 via-black/68 to-black/42" />

        <div className={`relative z-10 mb-4 overflow-hidden rounded-[22px] bg-[var(--bg-secondary)] ring-1 ring-white/10 md:mb-0 ${rank === 0 ? 'h-[245px]' : 'h-[210px]'}`}>
          {localizedItem.cover_image ? (
            <img src={localizedItem.cover_image} alt={localizedItem.title} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" loading={rank === 0 ? 'eager' : 'lazy'} onError={retryWithProxy} />
          ) : (
            <div className="flex h-full w-full items-center justify-center text-[var(--text-muted)]"><Icon size={44} /></div>
          )}
        </div>

        <div className="relative z-10 flex min-h-[210px] flex-col justify-between md:min-h-0">
          ...
          <div>
            ...
            <h3 className="line-clamp-3 font-display text-[28px] font-black leading-[0.95] tracking-[-0.045em] text-white md:text-[34px]">{localizedItem.title}</h3>
  ...
  return (
    <>
      <button
        ...
        onClick={() => setDrawerMedia(drawerPayload)}
        data-no-swipe="true"
        className={`group flex w-full items-center gap-3 rounded-[20px] border bg-[var(--bg-card)] text-left ${compact ? 'p-2' : 'p-2.5'} transition-all hover:bg-[var(--bg-card-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]/35 ${
          rank === 0 && !compact
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="gk-mono mt-1 text-[var(--text-muted)]">{locale === 'en' ? 'open details' : 'apri dettagli'}</p>
        </div>
