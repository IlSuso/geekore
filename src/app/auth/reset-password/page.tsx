...
const resetCopy = {
  it: {
    homeAria: 'Home Geekore',
    login: 'Login',
    tooShort: 'La password deve essere almeno 8 caratteri.',
    mismatch: 'Le password non coincidono.',
    resetError: 'Errore nel reset. Il link potrebbe essere scaduto.',
    doneKicker: 'Password aggiornata',
    doneTitle: 'Tutto pronto',
    doneBody: 'Ti stiamo riportando al login.',
    kicker: 'Nuova password',
    title: 'Proteggi l’account',
    body: 'Scegli una password nuova. Dopo il salvataggio torni al login.',
    password: 'Nuova password',
    passwordPlaceholder: 'Almeno 8 caratteri',
    showPassword: 'Mostra password',
    hidePassword: 'Nascondi password',
    strength: 'Forza password',
    strengthLabels: ['', 'debole', 'media', 'buona', 'forte'],
    confirm: 'Conferma password',
    confirmPlaceholder: 'Ripeti la password',
    hint: 'Usa almeno 8 caratteri. Meglio se include lettere maiuscole, numeri e simboli.',
    saving: 'Aggiornamento...',
    submit: 'Aggiorna password',
  },
  en: {
    homeAria: 'Geekore home',
    login: 'Login',
    tooShort: 'Password must be at least 8 characters.',
    mismatch: 'Passwords do not match.',
    resetError: 'Could not reset the password. The link may have expired.',
    doneKicker: 'Password updated',
    doneTitle: 'All set',
    doneBody: 'We are taking you back to login.',
    kicker: 'New password',
    title: 'Protect your account',
    body: 'Choose a new password. After saving, you will return to login.',
    ...
    hint: 'Use at least 8 characters. It is better if it includes uppercase letters, numbers, and symbols.',
    saving: 'Updating...',
...
function ResetContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) setError(copy.resetError)
    else {
      setDone(true)
    ...
    return (
      <ResetShell>
        <div className="w-full max-w-[430px] rounded-[28px] border border-[var(--border)] bg-[rgba(20,20,28,0.78)] p-7 text-center shadow-[0_24px_80px_rgba(0,0,0,0.34)] ring-1 ring-white/5 backdrop-blur-xl">
          ...
          <p className="gk-label mb-2 text-emerald-300">{copy.doneKicker}</p>
          <h1 className="font-display text-[30px] font-black leading-none tracking-[-0.045em] text-[var(--text-primary)]">{copy.doneTitle}</h1>
          <p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">{copy.doneBody}</p>
        </div>
      </ResetShell>
    )
  }
    ...
    <ResetShell>
      <div className="w-full max-w-[430px] overflow-hidden rounded-[28px] border border-[var(--border)] bg-[rgba(20,20,28,0.78)] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.34)] ring-1 ring-white/5 backdrop-blur-xl md:p-7">
        <div className="mb-6">
          ...
          <p className="gk-label mb-2 text-[var(--accent)]">{copy.kicker}</p>
          <h1 className="font-display text-[32px] font-black leading-none tracking-[-0.045em] text-[var(--text-primary)]">{copy.title}</h1>
          <p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">{copy.body}</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          <div>
            <label htmlFor="password" className="mb-2 block text-[11px] font-black uppercase tracking-[0.12em] text-[var(--text-muted)]">{copy.kicker}</label>
            <div className="relative">
              ...
              />
              <button type="button" onClick={() => setShowPw(v => !v)} className="absolute right-3 top-1/2 grid h-8 w-8 -translate-y-1/2 place-items-center rounded-xl text-[var(--text-muted)] transition-colors hover:bg-white/[0.04] hover:text-white" aria-label={showPw ? copy.hidePassword : copy.showPassword}>
            ...
            {password.length > 0 && (
              <div className="mt-2">
                ...
                <p className="mt-1 font-mono-data text-[10px] text-[var(--text-muted)]">{copy.strength}: {copy.strengthLabels[score]}</p>
              </div>
            )}
          </div>

          <div>
            <label htmlFor="confirm" className="mb-2 block text-[11px] font-black uppercase tracking-[0.12em] text-[var(--text-muted)]">{copy.confirm}</label>
            <input
              id="confirm"
              type={showPw ? 'text' : 'password'}
              value={confirm}
            ...
            />
            {confirm.length > 0 && !passwordsMatch && <p className="mt-2 text-xs text-red-300">{copy.mismatch}</p>}
          </div>
          ...
          <div className="rounded-2xl border border-[var(--border-subtle)] bg-black/18 p-4">
            <div className="flex gap-3">
              ...
              <p className="text-xs leading-5 text-[var(--text-muted)]">{copy.hint}</p>
            </div>
          </div>
