...
export default function OnboardingPage() {
  ...
  const completeOnboarding = useCallback(async () => {
    ...
    // Update base — onboarding_done è il campo critico
    await supabase.from('profiles').update({
      onboarding_done: true,
      onboarding_step: 3,
    }).eq('id', uid)
    // preferred_types in update separato — la colonna potrebbe non esistere ancora
    if (selectedTypes.length > 0) {
      await supabase.from('profiles').update({
        preferred_types: selectedTypes,
      }).eq('id', uid).then(() => {}) // ignora errore se colonna mancante
    }
    setOnboardingCookie()
