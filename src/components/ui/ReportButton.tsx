...
// Bottone di segnalazione contenuti — si usa su post, commenti, profili.
...
export function ReportButton({
  targetType, targetId, iconOnly = false, className = '',
}: ReportButtonProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (error) {
      if (error.code === '23505') {
        showToast('Hai già segnalato questo contenuto', 'error')
      } else {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl w-full max-w-sm p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button
                onClick={() => { setOpen(false); setReason(null); setNotes('') }}
