...
const MOBILE_NOTIFICATIONS_COPY = {
  it: {
    title: "Notifiche",
    close: "Chiudi notifiche",
    newCount: (n: number) => `${n} nuove`,
    openPage: "Apri pagina",
    emptyTitle: "Nessuna notifica",
    emptyBody: "Quando qualcuno interagisce con te, lo vedrai qui.",
    someone: "Qualcuno",
    labels: {
      like: (actor: ReactNode) => (
        <>
          <span className="font-black text-[var(--text-primary)]">{actor}</span>{" "}
          ha messo like al tuo post
        </>
      ),
      comment: (actor: ReactNode) => (
        <>
          <span className="font-black text-[var(--text-primary)]">{actor}</span>{" "}
          ha commentato il tuo post
        </>
      ),
      follow: (actor: ReactNode) => (
        <>
          <span className="font-black text-[var(--text-primary)]">{actor}</span>{" "}
          ha iniziato a seguirti
        </>
      ),
      integration: () => <>La tua integrazione ha sincronizzato nuovi media</>,
      default: () => <>Geekore ha una nuova raccomandazione per te</>,
    },
  },
  en: {
    title: "Notifications",
    ...
    emptyBody: "When someone interacts with you, you will see it here.",
    someone: "Someone",
    labels: {
      like: (actor: ReactNode) => (
        <>
      ...
      integration: () => <>Your integration synced new media</>,
      default: () => <>Geekore has a new recommendation for you</>,
    },
  },
} as const;

type MobileNotificationsCopy =
  (typeof MOBILE_NOTIFICATIONS_COPY)[keyof typeof MOBILE_NOTIFICATIONS_COPY];
...
function NotifIcon({ type }: { type: string }) {
  if (type === "like")
    return <Heart size={13} className="text-red-400" fill="currentColor" />;
  if (type === "comment")
    return (
      <MessageCircle
        size={13}
  ...
  if (type === "follow")
    return <UserPlus size={13} style={{ color: "var(--accent)" }} />;
  if (type === "integration")
    return <PlugZap size={13} className="text-sky-300" />;
...
function notifLabel(
  type: string,
  actor: string,
  copy: MobileNotificationsCopy,
) {
  if (type === "like") return copy.labels.like(actor);
  if (type === "comment") return copy.labels.comment(actor);
  if (type === "follow") return copy.labels.follow(actor);
  if (type === "integration") return copy.labels.integration();
  return copy.labels.default();
}

function compactTime(date: string, locale: "it" | "en") {
...
export function MobileNotificationsDrawer({
  ...
  const copy = MOBILE_NOTIFICATIONS_COPY[locale];
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      supabase
        .from("notifications")
        .select(
          "id, type, is_read, created_at, post_id, actor:actor_id(username, display_name, avatar_url)",
        )
      ...
      >
        <div className="flex flex-shrink-0 flex-col gap-3 border-b border-[var(--border)] bg-[rgba(11,11,15,0.92)] px-4 pb-3 pt-3 backdrop-blur-2xl">
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              <h1 className="font-display text-[22px] font-black tracking-[-0.03em] text-[var(--text-primary)]">
                {copy.title}
              </h1>
              <p className="gk-section-eyebrow">{copy.newCount(unread)}</p>
            </div>
            <Link
              href="/notifications"
              onClick={doClose}
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain p-3">
          {loading && (
            <div className="flex flex-col gap-2">
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 rounded-[20px] border border-[var(--border-subtle)] bg-[var(--bg-card)] p-3 animate-pulse"
                >
          ...
          {!loading && notifications.length === 0 && (
            <div className="gk-empty-state mt-8">
              ...
              <p className="gk-empty-state-title">{copy.emptyTitle}</p>
              <p className="gk-empty-state-subtitle">{copy.emptyBody}</p>
            </div>
          )}
          ...
          {!loading && notifications.length > 0 && (
            <div className="space-y-2">
              {notifications.map((notif) => {
                ...
                const name =
                  actor?.display_name || actor?.username || copy.someone;
                return (
                  <div
                    key={notif.id}
                  ...
                  >
                    <span
                      className={`h-1.5 w-1.5 flex-shrink-0 rounded-full ${notif.is_read ? "bg-transparent" : "bg-[var(--accent)]"}`}
                    />
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[13.5px] leading-snug text-[var(--text-secondary)]">
                        {notifLabel(notif.type, name, copy)}
                      </p>
                      <p className="mt-1 gk-mono text-[var(--text-muted)]">
                        {compactTime(notif.created_at, locale)}
                      </p>
                    </div>
                  </div>
                );
