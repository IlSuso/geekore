...
function RailCard({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <section className={`rounded-[24px] border border-[var(--border-subtle)] bg-[rgba(18,18,25,0.72)] p-4 ring-1 ring-white/5 ${className}`}>
      {children}
    </section>
  )
}

function RailHeader({ icon, title, href }: { icon: React.ReactNode; title: string; href?: string }) {
  return (
    <div className="mb-3 flex items-center justify-between gap-3">
      <div className="flex min-w-0 items-center gap-2">
        <span className="text-[var(--accent)]">{icon}</span>
        <p className="truncate text-[12px] font-black uppercase tracking-[0.06em] text-[var(--text-secondary)]">{title}</p>
      </div>
      {href && <Link href={href} className="gk-mono flex-shrink-0 text-[var(--accent)]">vedi</Link>}
    </div>
  )
}
...
function PulseCard({ currentUserId }: { currentUserId: string | null }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase.from('follows').select('id', { count: 'exact', head: true }).eq('follower_id', currentUserId),
  ...
  return (
    <RailCard className="bg-[linear-gradient(135deg,rgba(230,255,61,0.055),rgba(18,18,25,0.72))]">
      <RailHeader icon={<Radio size={14} />} title="Community pulse" />
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-2xl bg-black/18 p-3">
          <p className="font-mono-data text-[18px] font-black leading-none text-[var(--accent)]">{counts.following}</p>
          <p className="gk-label mt-1">seguiti</p>
        </div>
        <div className="rounded-2xl bg-black/18 p-3">
          <p className="font-mono-data text-[18px] font-black leading-none text-[var(--text-primary)]">{counts.posts}</p>
          <p className="gk-label mt-1">post 7g</p>
        </div>
      </div>
    </RailCard>
  )
            ...
            >
              ...
              <div className="h-[124px] w-[88px] shrink-0 overflow-hidden rounded-[18px] bg-[var(--bg-elevated)] ring-1 ring-white/10">
                {item.cover_image
                  ...
                  : <div className="flex h-full w-full items-center justify-center"><Icon size={22} className="text-[var(--text-muted)]" /></div>}
              </div>
              <div className="min-w-0">
                <p className="line-clamp-3 text-[14.5px] font-black leading-tight text-[var(--text-primary)]">{item.title}</p>
                <p className="mt-1.5 text-[11px] font-bold leading-tight text-[var(--text-muted)]">{CATEGORY_LABEL[item.type] || item.type}</p>
                <p className="mt-1 inline-flex rounded-full border border-[rgba(230,255,61,0.16)] bg-[rgba(230,255,61,0.06)] px-2 py-0.5 font-mono-data text-[10px] font-black text-[var(--accent)]">{item.count} attività</p>
              </div>
            </Link>
          )
        })}
...
function SuggestedUsersCard({ currentUserId }: { currentUserId: string }) {
  ...
  useEffect(() => {
    ...
    const fetchSuggested = async () => {
      ...
      setUsers(((data || []).filter((u: any) => !followingIds.has(u.id)).slice(0, 4)))
    }
    fetchSuggested()
  }, [currentUserId])
  ...
  return (
    <RailCard>
      <RailHeader icon={<Users size={14} />} title="Da seguire" href="/friends" />
      <div className="space-y-2.5">
        {users.map((user, index) => (
          <div key={user.id} className="flex items-center gap-2.5">
            <Link href={`/profile/${user.username}`} className="shrink-0">
              <Avatar src={user.avatar_url} username={user.username} displayName={user.display_name} size={36} className="rounded-2xl" />
            </Link>
            <div className="min-w-0 flex-1">
              ...
              <p className="gk-mono text-[var(--text-muted)]">match {92 - index * 7}%</p>
            </div>
            {followed.has(user.id) ? (
              <span className="rounded-full border border-[var(--border)] px-2 py-1 text-[10px] font-bold text-[var(--text-muted)]">Seguito</span>
            ) : (
              <button onClick={() => handleFollow(user.id)} className="rounded-full border border-[rgba(230,255,61,0.45)] px-2.5 py-1 text-[10px] font-black text-[var(--accent)] transition-colors hover:bg-[rgba(230,255,61,0.08)]">
                Segui
              </button>
            )}
          </div>
