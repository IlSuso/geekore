...
import { Brain, ChevronDown, ChevronUp, Flame, Search, Bookmark, Sparkles, Star } from 'lucide-react'
...
const MEDIA_ORDER: MediaType[] = ['anime', 'game', 'tv', 'manga', 'movie', 'boardgame']
...
export const DNAWidget = memo(function DNAWidget({ tasteProfile, profile: legacyProfile, totalEntries, compact = false }: DNAWidgetProps) {
  ...
  const dominantGenres = profile.globalGenres
    .filter((g, index, arr) => arr.findIndex(x => x.genre === g.genre) === index)
    .slice(0, 4)

  const collectionTotal = Object.values(profile.collectionSize || {}).reduce((sum, value) => sum + (Number(value) || 0), 0)
  const mediaBreakdown = MEDIA_ORDER
    .map(type => ({ type, count: Number(profile.collectionSize?.[type] || 0) }))
    ...
    <section className={`mb-8 overflow-hidden rounded-[28px] border border-[rgba(230,255,61,0.20)] bg-[linear-gradient(135deg,rgba(230,255,61,0.08),rgba(139,92,246,0.07),rgba(20,20,27,0.92))] shadow-[0_18px_60px_rgba(0,0,0,0.28)] ${compact ? 'md:mb-6' : ''}`}>
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-[rgba(230,255,61,0.35)] bg-[rgba(230,255,61,0.08)] px-3 py-1 font-mono-data text-[10px] font-black uppercase tracking-[0.12em] text-[var(--accent)]">
              <Star size={11} fill="currentColor" /> Il tuo DNA
            </div>
      ...
      <div className="px-5 pb-5 md:px-6">
          ...
          <div className="mb-4 flex h-2.5 overflow-hidden rounded-full bg-black/30 ring-1 ring-white/5">
            {dominantGenres.map(({ genre, score }, index) => (
              <div
                ...
                style={{
                  width: `${Math.max(8, formatPercent(score, dominantGenres.reduce((sum, g) => sum + g.score, 0)))}%`,
                  background: ['var(--accent)', 'var(--type-anime)', 'var(--type-game)', 'var(--type-movie)'][index % 4],
                }}
              />
            ))}
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {mediaBreakdown.length > 0 ? mediaBreakdown.map(({ type, count }) => (
            <span key={type} className={`gk-chip ${MEDIA_CLASS[type]}`}>
              {MEDIA_LABEL[type]} {formatPercent(count, collectionTotal)}%
            </span>
          )) : (
            <span className="gk-chip gk-chip-match">Aggiungi media per calcolare il DNA</span>
          )}
        </div>
      </div>
      ...
      {open && (
        <div className="space-y-6 border-t border-[rgba(255,255,255,0.06)] px-5 pb-5 pt-5 md:px-6">
          {profile.globalGenres.length > 0 ? (
            <div>
              ...
              <div className="space-y-2.5">
                {profile.globalGenres.slice(0, 6).map(({ genre, score }, index) => {
                  const pct = Math.round((score / maxGenreScore) * 100)
                  return (
                    <div key={`global-${genre}-${index}`} className="flex items-center gap-3">
                      ...
                      <span className="w-8 text-right font-mono-data text-[10px] font-bold text-zinc-400">{pct}%</span>
                    </div>
                  )
                })}
              </div>
          ...
          {(bingeGenres.length > 0 || searchIntentGenres.length > 0 || wishlistGenres.length > 0 || discoveryGenres.length > 0) && (
            <div>
              <p className="gk-label mb-3">Segnali recenti</p>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
